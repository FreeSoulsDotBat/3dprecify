...
(typed transport wrapper, deploy env wiring + `docs/environments.md`, `/me` identity, FE Sentry). Current
increment: **006-uat-deploy-hardening** — first public UAT deploy (T022/FR-010) + FR-209 first release merge
`develop`→`main` + A21 error contract/Schemathesis + D4 `gate:all` + privacy notice. Still open elsewhere:
005 T042 (design reconciliation, non-blocking) + D1–D4 ML ingestion (blocked on the house ML account, Q-D).
