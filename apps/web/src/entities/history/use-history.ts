...
export function useSyncOutbox(
    ...
    const mutation = useMutation({
        ...
        onSettled: () => {
            void client.invalidateQueries({ queryKey: historyQueryPrefix(uid) });
            // ⚠ @doc DEC-097 — uma query `networkMode: "always"` que errou NÃO se cura na
            //   reconexão (o `refetchOnReconnect` pega carona na maquinaria que o "always"
            //   dispensa). Este dreno tem de invalidar a MESMA chave, ou o detalhe fica preso.
            void client.invalidateQueries({ queryKey: historyDetailPrefix(uid) });
            void client.invalidateQueries({ queryKey: outboxQueryKey(uid) });
        },
    });
