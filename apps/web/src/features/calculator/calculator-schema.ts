    ...
    return z.string().transform((raw, ctx): number => {
        ...
        if (rule.positive) {
            if (n <= 0) {
                ctx.addIssue({
                    code: "custom",
                    message: rule.positiveMessage ?? t.rollWeightError,
                });
                return z.NEVER;
            }
        } else if (n < 0) {
        ...
        if (rule.ltExclusive !== undefined && n >= rule.ltExclusive) {
            ctx.addIssue({
                code: "custom",
                message: rule.ltExclusiveMessage ?? t.validation.invalid,
            });
            return z.NEVER;
        }
        return n;
...
export const calculatorSchema = z.object({
    ...
    rollWeightKg: numField({
        kind: "required",
        positive: true,
        positiveMessage: t.rollWeightError,
    }),
