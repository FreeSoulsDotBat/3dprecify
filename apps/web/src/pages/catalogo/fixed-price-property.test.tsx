...
// (`features/catalog/**`) e a ficha (`pages/catalogo/**`). Qualquer outro arquivo de `src/` que
...
// podem mostrar um preço), MENOS `pages/catalogo/**`/`features/catalog/**` (as exceções) e MENOS
...
describe("019/PR-D (T125) — a guarda de propriedade: seller_fixed_price nunca vaza para fora do recálculo", () => {
  ...
  it("orçamento (pages/historico/**) não lê sellerFixedPrice/observedPrice/price-observations", () => {
    expect(violations("pages/historico")).toEqual([]);
  });
  ...
  it("o resto de pages/**+features/** (fora de pages/catalogo/** e features/catalog/**) também não lê", () => {
    const EXEMPT = new Set(["pages/catalogo", "features/catalog"]);
...
describe("019/PR-D (T125) — o render: PriceHero vem de recomputed ou sellerFixedPrice, nunca de observedPrice (caso não-degradado)", () => {
  it("uma observação DIVERGENTE não vaza para o número grande — o número é o recomputado de hoje", () => {
    ...
    useObservationsMock.mockReturnValue({
      byKey: new Map([
        ["PRODUCT:prod-1", { observedPrice: 999, observedAt: "2026-05-01", subjectId: "prod-1" }],
      ]),
      isLoading: false,
      isError: false,
      error: null,
    ...
    render(
      <QueryClientProvider client={client}>
        <ProdutoPage productId="prod-1" gate="active" />
      </QueryClientProvider>,
    );
