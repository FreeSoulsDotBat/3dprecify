...
describe("M11 — the per-channel prices are rendered, not silently frozen (review PR-A)", () => {
    ...
    it("nomeia o marketplace como a Calcular nomeia — nunca o enum cru (R3)", () => {
        render1(
            ...
            {
                ...PAYLOAD,
                channels: [{ marketplace: "MERCADO_LIVRE", precoAnuncioVarejo: "56.51" }],
            },
        );
    ...
    it("um valor fora do dicionário passa intacto — o congelado não é reescrito", () => {
        render1(
            ...
            {
                ...PAYLOAD,
                channels: [{ marketplace: "Loja própria", precoAnuncioVarejo: "10.00" }],
            },
        );
...
describe("the technical sheet — the date and the formula version, labelled (A29 / FR-506)", () => {
    it("names the formula version the price was computed with", () => {
        render1();
        expect(
            screen.getByText(t.modelVersionLine.replace("{versao}", "3.1.0")),
        ).toBeInTheDocument();
    });
