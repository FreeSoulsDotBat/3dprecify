 ...
 *
 *  Exportada em 019/PR-E (T088) — `pages/history/quote-line-input.ts` reusa esta MESMA leitura
 *  D3/D6 para o construtor de orçamento (mesma pasta `pages/history`, sem cruzar fronteira). */
...
export function recalcToday(
    ...
    if (frozen.kind === "KIT" && origin.kit && provenance) {
        ...
        const lines = mapped.filter(
            (l): l is { input: PriceInput; quantity: number; name: string | null } =>
                l.input !== null,
        );
        ...
        if (lines.length > 0 && lines.length === mapped.length) {
            const { bom } = composeBom(
                lines.map((l) => ({ input: l.input, quantity: l.quantity })),
            );
            return {
                payload: freezeBomResult(
                    lines,
                    bom,
                    provenance,
                    ctx.catalog.catalogVersion ?? null,
                ),
                fromFrozen: false,
            };
        }
        ...
        <>
            ...
            <Dialog open={open} onOpenChange={(next) => !next && setOpen(false)}>
                <DialogContent showClose={false}>
                    ...
                    <DialogDescription>
                        {repriced
                            ? t.recalcBody.replace("{data}", quotedDate)
                            : t.recalcNoOriginBody}
                    </DialogDescription>
