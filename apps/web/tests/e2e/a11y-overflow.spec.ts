...
test.describe("no horizontal overflow at 360px — public surfaces", () => {
    for (const theme of THEMES) {
        test(`/calcular has no horizontal overflow (${theme})`, async ({ page }) => {
            ...
            await expect(
                page.getByRole("heading", { name: messages.calculator.title }),
            ).toBeVisible();
        ...
        test(`/calcular has no horizontal overflow with giant values (${theme})`, async ({
            page,
        }) => {
            ...
            await page
                .getByLabel(messages.calculator.fields.costPerRoll)
                .fill("999999999999999,99");
        ...
        test(`branded 404 has no horizontal overflow (${theme})`, async ({ page }) => {
            ...
            await expect(
                page.getByRole("heading", { name: messages.notFound.title }),
            ).toBeVisible();
            await setTheme(page, theme);
            const { scrollWidth, clientWidth } = await overflow(page);
