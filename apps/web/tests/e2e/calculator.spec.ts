...
test("US1: o seletor de categoria é um campo de primeira classe, não um <input> cru (FR-006a, T115)", async ({
    ...
    expect(doSeletor, "moldura do seletor == moldura do campo de dinheiro irmão").toEqual(
        doDinheiro,
    );
    ...
    await expect
        .poll(() => moldura(escolhido), {
            message: "moldura do escolhido == moldura do campo irmão",
        })
        .toEqual(doDinheiro);
});
...
test("a promessa 'é grátis' está na primeira dobra, não no rodapé (360px)", async ({ page }) => {
    ...
    expect(
        y.altura,
        "a pagina precisa ser rolavel para a assercao significar algo",
    ).toBeGreaterThan(800 * 2);
    expect(
        y.topo,
