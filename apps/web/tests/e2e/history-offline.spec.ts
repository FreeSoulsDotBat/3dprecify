...
test("a record made offline is pending, survives a reload, then drains to synced (B1/FR-527)", async ({
  ...
  await expect(page.getByText(t.syncPendingBadge)).toBeVisible();
  ...
  await expect(page.getByText(t.quotedValue)).toBeVisible();
  await expect(page.locator(".tf-historico__money strong").first()).toHaveText(quoted);
  await expect(page.getByText(t.frozenExplainer)).toBeVisible();
  await expect(page.getByText(t.deviceClockNote)).toBeVisible();
});
