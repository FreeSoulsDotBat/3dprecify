...
async function createProduct(
  ...
  await page.getByRole("combobox", { name: catalogo.printer }).selectOption({ label: printerName });
...
test("US16: 3 itens × (1, 2, 10) — o total bate com computeQuote rodado fora da tela", async ({
  ...
  await page.getByRole("dialog").getByRole("button", { name: t.historico.exportGenerate }).click();
  await quoteRequest;
...
test("DECISÃO 4: offline monta mas não envia — reconectar habilita e grava UMA vez", async ({
  ...
  await createProduct(page, "Peca Offline", "Filamento Offline", "Impressora Offline", "100", "4");
      ...
      new Promise<string[]>((resolve, reject) => {
        ...
        req.onsuccess = () => {
          ...
          cursorReq.onsuccess = () => {
            ...
            if (cursor) {
              if (String(cursor.key).startsWith("history:outbox:")) keys.push(String(cursor.key));
              cursor.continue();
            } else resolve(keys);
          };
