...
test.describe("D3/D6 — a scenario referencing a saved product", () => {
    test("D3 live-reflect: editing the linked filament's cost changes the reopened scenario's price (SC-601/VR-605)", async ({
        ...
        if ((page.viewportSize()?.width ?? 0) >= 1280) {
            ...
            await editPanel
                .getByRole("textbox", { name: new RegExp(t.fields.costPerRoll) })
                .fill("220");
        ...
        await expect(page.getByTestId("scenario-context-bar")).not.toContainText(
            pf.manualValuesKept,
        ); // never degraded — the ref still resolves
