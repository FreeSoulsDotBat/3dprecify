...
export async function aplicarSubcenario(
    ...
    await page.addInitScript((th: string) => {
        localStorage.setItem(
            "precifica3d-theme",
            JSON.stringify({ state: { theme: th }, version: 0 }),
        );
...
export async function varreduraDeSaude(
    ...
    if (ov.y > alturaJanela * 6) {
        defeito(info, {
            ...
            resultado_esperado:
                "Conteúdo alcançável em poucas telas, ou um resumo que não exige rolar",
    ...
    if (semRotulo.length > 0) {
        defeito(info, {
            ...
            resultado_esperado:
                "Todo campo tem <label for>, aria-label ou envolvimento por <label>",
