...
test("CF-001-UI-01 — mobile dark anônimo: aritmética da semente e resistência dos campos a lixo", async ({
    ...
    for (const k of [
        ...
        if (tela[k] === null) {
            ...
            if (
                Math.abs((tela[k] as number) - (esperado[k as keyof typeof esperado] as number)) >
                0.005
            ) {
                defeito(info, {
                    subcenario: SUB,
    ...
    if (tela.material !== null && tela.custoTotal !== null) {
        ...
        expect
            .soft(soma, `${SUB}/T02 soma das linhas == custo total`)
            .toBeCloseTo(tela.custoTotal, 2);
    }
    executado(info, SUB, "calculo");
    ...
    const lixos: { rotulo: string; valor: string; nota: string }[] = [
        ...
        {
            rotulo: t.calculator.fields.costPerRoll,
            valor: "1e308",
            nota: "notação científica gigante",
        },
...
test("CF-001-UI-03 — 1024px premium: a faixa do meio não regride o cálculo nem o layout", async ({
    ...
    if (temToggleRail) {
        defeito(info, {
            ...
            categoria: "layout",
            descricao:
                "Botão de recolher a barra lateral aparece a 1024px, abaixo do corte de 1280px",
