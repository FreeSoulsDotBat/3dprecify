  ...
  if (!(await volume.isVisible().catch(() => false))) {
    ...
    if (todosIguais) {
      defeito(info, {
        ...
        resultado_esperado: "Perfis diferentes selecionam entradas diferentes do catálogo (FR-926)",
...
test("CF-008-UI-01 — categoria: contagem honesta, busca adversarial e limpar", async ({
  ...
  if ((await busca.count()) === 0) {
    defeito(info, {
      ...
      resultado_esperado: "Busca textual sobre as categorias (o dono pediu na rodada 1, ponto 10)",
    ...
    for (const termo of ["a", "casa", "ele", "ç", "🎉", "'; DROP TABLE", "z".repeat(120)]) {
      ...
      if (/NaN|\[object Object\]|Traceback|TypeError/.test(corpo)) {
        defeito(info, {
          ...
          resultado_obtido: /NaN|\[object Object\]|Traceback|TypeError/.exec(corpo)?.[0] ?? "",
          severidade: "alta",
        });
      }
      ...
      if (m) {
        ...
        const botoes = await page.getByRole("button", { name: /›|\//, exact: false }).count();
        const exibidos = Math.max(opcoes, 0);
        if (exibidos > 0 && declarado > exibidos && !/mostrando|mais de|primeiras/i.test(corpo)) {
          defeito(info, {
            subcenario: SUB,
            categoria: "acolhimento",
...
test("CF-025-UI-03 — sessão expira com a fila cheia: o registro FICA e a tela diz a causa certa", async ({
  ...
  if (!aindaTemRegistro) {
    defeito(info, {
      ...
      resultado_esperado: "401 NUNCA purga a fila (hotfix 016/A3): o registro continua no aparelho",
...
test("CF-024-UI-01 — apagar do catálogo a peça de um kit salvo: a linha degrada de forma honesta", async ({
  ...
  if (tinhaPreco && !/R\$/.test(depois)) {
    defeito(info, {
      ...
      resultado_esperado: "A linha degradada mantém o último valor conhecido e o kit segue somando",
