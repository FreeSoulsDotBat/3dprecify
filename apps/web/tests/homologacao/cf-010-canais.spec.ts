    ...
    (
      await page.evaluate(() =>
        Array.from(document.querySelectorAll("input,select")).map((el) => {
          ...
          const lab = i2.id ? document.querySelector(`label[for="${CSS.escape(i2.id)}"]`) : null;
...
test("CF-010-A — a conta do canal fecha com o que está na tela, em 24 combinações de tarifa", async ({
  ...
  const temComissao = await (await campoCanal(page, ch.commission)).isVisible().catch(() => false);
  ...
  for (const combo of combos) {
    ...
    if (l.anuncio < l.varejoBase - 0.01) {
      defeito(info, {
        ...
        resultado_esperado: "Anúncio sempre >= preço-base (a comissão só pode empurrar para cima)",
...
test("CF-010-C — um canal com comissão inválida não pode apagar o preço do outro", async ({
  ...
  if ((await comissoes.count()) < 2) {
    defeito(info, {
      subcenario: SUB,
      categoria: "fluxo",
      descricao: "Adicionar um segundo canal não criou um segundo conjunto de campos de tarifa",
  ...
  if (quantosAnuncios === 0) {
    defeito(info, {
      ...
      resultado_esperado: "O canal válido continua mostrando anúncio e líquido; só o inválido erra",
  ...
  if (!recusou) {
    ...
    const entorno = await page.evaluate(() => {
      const marca = Array.from(document.querySelectorAll("input")).filter((i) => i.value === "150");
      return marca.map((i) => {
...
test("CF-010-D — tarifas vindas do catálogo: procedência declarada e conta coerente", async ({
  ...
  const temSelo = /catálogo|catalogo|tabela|atualizad|referência|referencia|informe as taxas/i.test(
    corpo,
  );
  if (!temSelo) {
    defeito(info, {
