    ...
    for (const tema of ["dark", "light"] as const) {
        test(`CF-043/046/047-UI — varredura de todas as rotas públicas em ${viewport}/${tema}`, async ({
            ...
            for (const rota of PUBLICAS) {
                ...
                if (ov.x > 1) {
                    defeito(info, {
                        ...
                        resultado_esperado:
                            "Nenhuma rolagem horizontal em nenhuma largura suportada",
            ...
            for (const inexistente of ["/rota-que-nao-existe", "/catalogo/nao-existe-mesmo"]) {
                ...
                const achou404 =
                    corpo.includes(t.notFound.title) || corpo.includes(t.notFound.body);
                if (!achou404) {
                    defeito(info, {
                        subcenario: SUB,
    ...
    for (const { rota, titulo } of TEASERS) {
        ...
        if (m) {
            defeito(info, {
                ...
                resultado_obtido: corpo
                    .slice(Math.max(0, m.index - 60), m.index + 90)
                    .replace(/\n/g, " "),
                severidade: "alta",
            });
        }
