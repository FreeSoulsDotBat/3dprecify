...
async function criarFilamento(page: Page, nome: string, custo = "110", peso = "1"): Promise<void> {
  ...
  await page
    ...
    .fill(custo);
  await page.getByRole("textbox", { name: new RegExp(t.calculator.fields.rollWeight) }).fill(peso);
...
test("CF-015-UI-01/02/03 — filamentos: estado vazio, criação, entradas absurdas e 30 itens", async ({
  ...
  if (!/adicionar|criar|comece|primeiro/i.test(vazio)) {
    defeito(info, {
      ...
      resultado_esperado: "Estado vazio com uma ação clara (ex.: adicionar o primeiro filamento)",
  ...
  if (quantosDup < 2) {
    ...
    if (!/já existe|ja existe|duplicad|mesmo nome/i.test(corpo)) {
      defeito(info, {
        subcenario: SUB,
        categoria: "crud",
        descricao: "Salvar um filamento com nome duplicado não criou o item nem explicou por quê",
...
test("CF-025/CF-026 — orçamento: registra online, registra offline e a fila é honesta", async ({
  ...
  if (!temRegistrar) {
    defeito(info, {
      ...
      descricao: "Um vendedor Premium não encontra a ação de registrar o orçamento na calculadora",
  ...
  if (!/R\$/.test(ledger)) {
    defeito(info, {
      ...
      categoria: "fluxo",
      descricao: "Depois de registrar um orçamento, a aba Orçamentos não mostra nenhum registro",
...
test("CF-035/036/037 — Conta: identidade honesta, plano correto e oferta sem overflow", async ({
  ...
  if (!dizPremium || dizGratuito) {
    defeito(info, {
      ...
      resultado_esperado: "Painel de plano dizendo Premium ativo (com validade, quando houver)",
...
test("CF-035-UI-02 — com /me e /entitlement falhando, a Conta é honesta e oferece retentativa", async ({
  ...
  if (!temRetentativa) {
    defeito(info, {
      subcenario: SUB,
      categoria: "acolhimento",
      descricao: "Com o servidor falhando, a Conta não oferece nenhum botão de tentar de novo",
...
test("CF-012-UI-01 — usar do catálogo preenche os campos com os valores salvos", async ({
  ...
  if (!(await seletor.isVisible({ timeout: 15_000 }).catch(() => false))) {
    defeito(info, {
      subcenario: SUB,
      categoria: "gate_premium",
      descricao: 'Um Premium com filamento salvo não vê o seletor "Filamento salvo" na calculadora',
      resultado_esperado: "Combobox de item salvo visível para conta com entitlement ativo e itens",
