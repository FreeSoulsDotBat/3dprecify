    ...
    if (await nomeKit.isVisible().catch(() => false)) {
        ...
        if (/Internal Server Error|500|Unprocessable|422|Traceback/i.test(aposSalvar)) {
            defeito(info, {
                ...
                resultado_obtido:
                    /Internal Server Error|500|Unprocessable|422|Traceback/i.exec(
                        aposSalvar,
                    )?.[0] ?? "",
                severidade: "alta",
            });
    ...
    if (await salvar.isVisible().catch(() => false)) {
        ...
        if (!corpo.includes(b.kitNameRequired)) {
            defeito(info, {
                ...
                categoria: "input_invalido",
                descricao:
                    "Salvar um kit sem nome não exibiu a mensagem que o produto define para o caso",
...
test("CF-013/CF-032 — simulação: nome vazio, 121 caracteres, nota longa e ida e volta completa", async ({
    ...
    if (mascaraDepois && mascaraAntes.includes(".") && !mascaraDepois.includes(".")) {
        defeito(info, {
            ...
            resultado_esperado:
                "Valor restaurado com a mesma formatação que o campo aplica no blur",
        ...
        for (const termo of [
            ...
            if (/NaN|\[object Object\]|Traceback|TypeError/.test(texto)) {
                defeito(info, {
                    ...
                    resultado_obtido:
                        /NaN|\[object Object\]|Traceback|TypeError/.exec(texto)?.[0] ?? "",
                    severidade: "alta",
                });
            }
        }
...
test("CF-013-UI-03 — free: sem ação de salvar, com oferta honesta e sem modal morto", async ({
    ...
    if (quantosSalvar > 0) {
        defeito(info, {
            ...
            resultado_esperado:
                "Ação ausente para quem não tem Premium; a oferta ocupa o lugar dela",
    ...
    if (await entrada.isVisible().catch(() => false)) {
        ...
        if (!temCta) {
            defeito(info, {
                ...
                categoria: "gate_premium",
                descricao:
                    "A folha de simulações do FREE não oferece caminho para assinar o Premium",
