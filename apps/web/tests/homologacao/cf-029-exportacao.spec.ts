...
test("CF-029-UI-01 — exportar PDF com rótulo adversarial: o arquivo chega e é um PDF real", async ({
  ...
  if (!(await registrarOrcamento(page, rotulo))) {
    defeito(info, {
      ...
      descricao: "Não foi possível registrar um orçamento para exportar (ação de salvar ausente)",
  ...
  if (temAcao) {
    ...
    if (await gerar.isVisible().catch(() => false)) {
      ...
      if (download) {
        ...
        const explicou = depois.includes(h.exportLapsed) || /premium|reative|assinar/i.test(depois);
        if (!explicou) {
          defeito(info, {
            subcenario: SUB,
            categoria: "acolhimento",
        ...
        if (/403|401|Forbidden|Unauthorized|Internal Server Error/i.test(depois)) {
          defeito(info, {
            subcenario: SUB,
            categoria: "acolhimento",
            descricao: "A recusa de exportação vazou código HTTP para a tela do vendedor",
            resultado_esperado: "Mensagem em pt-BR, sem código de protocolo",
            resultado_obtido:
              /403|401|Forbidden|Unauthorized|Internal Server Error/i.exec(depois)?.[0] ?? "",
            severidade: "media",
          });
        }
      }
    ...
    if (download) {
      ...
      if (!bytes.subarray(0, 5).toString("latin1").startsWith("%PDF-")) {
        defeito(info, {
          subcenario: SUB,
          categoria: "acolhimento",
          descricao: "Offline, o produto entregou um download que NÃO é um PDF — um arquivo falso",
      ...
      const corpo = await page.locator("body").innerText();
      const honesto =
        corpo.includes(h.exportOffline) || /sem conexão|sem conexao|offline|conexão/i.test(corpo);
      if (!honesto) {
        defeito(info, {
          subcenario: SUB,
          categoria: "acolhimento",
          descricao: "Offline, gerar o PDF não produziu arquivo nem explicação — a tela ficou muda",
