...
test("LEIGO-B — formatação: '1,000' e '1.000' significam mil para o vendedor. E para o produto?", async ({
  ...
  for (const campo of campos) {
    for (const f of FORMATOS_REAIS) {
      ...
      if (RETRATADO_VER_SC305 && f.valor === "1,000" && campo === t.calculator.fields.costPerRoll) {
    ...
    if (persona.id === "P5" && p !== null && p === 0) {
      ...
      if (aviso === null) {
        defeito(info, {
          ...
          resultado_esperado: "Dizer que um preço zero não é vendável antes de entregá-lo",
    ...
    if (p !== null && referencia !== null && referencia > 0) {
      ...
      if (fator >= 100 && (await houveAviso(page)) === null) {
        defeito(info, {
          ...
          resultado_esperado: "Aviso quando o resultado sai da faixa que um vendedor reconheceria",
          resultado_obtido: `custo total R$ ${p.toFixed(2)}`,
          severidade: "alta",
        });
      }
...
test("LEIGO-E — jornadas reais: recarrega, volta, abre outra aba, desiste e retoma", async ({
  ...
  if (voltou.trim().length < 20) {
    defeito(info, {
      subcenario: SUB,
      categoria: "estabilidade",
      descricao: "Voltar pelo botão do navegador depois de trocar de aba deixou a tela em branco",
  ...
  if (precoNaDobra && precoNaDobra.topo > precoNaDobra.altura * 3) {
    defeito(info, {
      ...
      resultado_esperado: "O resultado alcançável em no máximo 2 a 3 telas, ou um resumo fixo",
