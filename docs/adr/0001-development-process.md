...
3. **CI & DoD enforcement** — **GitHub Actions** runs lint + type-check + tests (Vitest, pytest, Playwright) and
   is a **required check** to merge. The local PostToolUse hook stays as fast feedback only. Each increment
   appends a **DoD evidence block** (test output, qa-produto screenshots/console, analyze result).
...
- Follow-ups: stand up CI workflow (extend as backend/web land); configure `main` branch protection after the
  first CI run; bootstrap + pricing-core are grandfathered on `main` as the base; remaining 001 work and all
  later increments use feature branches.
