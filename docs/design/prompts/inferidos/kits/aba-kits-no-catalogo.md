...
- **Onde vive:** A quarta pílula do grupo segmentado do Catálogo (/catalogo?tab=kits) — ao lado de Filamentos · Impressoras · Produtos. No desktop ≥1280px o painel é um mestre-detalhe: lista à esquerda, ficha fixa de 560px à direita. Abaixo do corte é uma lista simples de cartões, sem ficha.
- **Como o vendedor chega:** Pelo botão "Ver meus kits" (que existe no vazio do compositor, no painel de Premium pausado e no recibo de materialização), ou tocando "Catálogo" na barra de abas e escolhendo a pílula "Kits". Ele vem consultar ou reabrir um kit que já montou.
- **Vizinhança imediata:** No desktop: barra de ferramentas com busca, a contagem "{n} kit(s)" e o botão "Montar kit"; abaixo, os cartões da lista, cada um com o nome do kit e, como resumo, "3 peça(s)" — nunca dinheiro, porque nenhum preço é guardado. À direita, a ficha do kit selecionado: o rótulo "Kit salvo", o nome em destaque, os botões de duplicar e excluir no canto superior direito, a MESMA frase "3 peça(s)" repetida no corpo e um botão "Abrir para editar". Nada mais — a ficha de 560px não edita nem mostra peças ou totais. No mobile: contagem + "Montar kit" no topo, e cartões com nome, resumo e três botões de ícone (lápis, copiar, lixeira). Com Premium pausado cada linha ganha "somente leitura"; com leitura de cache, "pode estar desatualizada".
- **Dados que chegam (e o que ela devolve):** A lista de kits do servidor com cache por conta (e a marca de leitura desatualizada quando o cache está servindo). De cada kit vêm nome, id e a contagem de peças. Devolve: navegação e exclusão.
- **O que acontece depois:** Abrir/editar leva a /kits?id=…, onde o compositor hidrata as peças e RECALCULA o preço do zero. Duplicar leva a /kits?id=…&copy=true, com o nome ganhando "(cópia)" e nada salvo até o vendedor mandar. "Montar kit" leva ao compositor vazio. Excluir pede confirmação; com Premium pausado, o toque intercepta honestamente em vez de fingir que funciona.
