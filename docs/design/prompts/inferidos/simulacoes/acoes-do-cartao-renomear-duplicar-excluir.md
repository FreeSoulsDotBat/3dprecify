...
- **Onde vive:** Última linha de CADA cartão de simulação, dentro da folha "Minhas simulações": um `flex items-center justify-end gap-1` — três `Button variant="ghost" size="sm"` encostados na borda direita do cartão, separados por apenas 4px, contendo só ícone de 18px e nenhum rótulo visível (lápis = Renomear, cópia = Duplicar, lixeira = Excluir, nessa ordem; o rótulo existe só como `aria-label`).
- **Como o vendedor chega:** Está permanentemente visível em todo cartão — não há menu "⋯", não há hover, não há swipe. O vendedor a encontra enquanto lê a lista, logo abaixo do carimbo "Atualizado há…".
- **Vizinhança imediata:** Imediatamente acima: o carimbo relativo cinza, último elemento do bloco clicável do cartão (tocar acima da linha ABRE a simulação; tocar na linha executa uma ação). Imediatamente abaixo: nada, ou — no congelamento de escrita — a linha cinza de justificativa alinhada à direita. À direita: a borda do cartão. Fora do cartão, 8px abaixo, o próximo cartão da lista.
- **Dados que chegam (e o que ela devolve):** Recebe o item do cartão e dois sinais de congelamento: `writesDisabled` (Premium pausado OU offline) e `writesReason` (a frase que explica qual dos dois). Devolve mutações: `PATCH` de renome, `POST /duplicate`, `DELETE` (soft).
- **O que acontece depois:** Lápis abre a folha "Renomear simulação" empilhada POR CIMA da folha da lista (nome + nota). Cópia executa a duplicação imediatamente, sem confirmação: toast "Simulação duplicada.", a folha fecha e a calculadora passa a exibir a CÓPIA. Lixeira abre um diálogo central de confirmação com corpo "Esta ação não pode ser desfeita." e um botão vermelho "Excluir"; confirmado, o cartão some da lista e um toast anuncia. Qualquer falha vira um alerta dentro da própria folha/diálogo, com os dados intactos.
