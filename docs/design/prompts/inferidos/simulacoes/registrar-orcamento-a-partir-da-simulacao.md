...
- **Onde vive:** Duas posições diferentes na MESMA rota `/calcular`, escolhidas pelo tipo de base da simulação aberta. (a) Base avulsa ou de produto: é o botão "Salvar no histórico" de sempre, no rodapé `tf-calc-footer`, centralizado, imediatamente ABAIXO do botão "Salvar simulação" — nada na tela indica que agora ele carrega a procedência da simulação. (b) Base KIT: aquele botão do rodapé é SUPRIMIDO e um botão idêntico aparece LÁ EM CIMA, centralizado logo abaixo do cartão de resumo do kit, a uma tela inteira de distância.
- **Como o vendedor chega:** O vendedor reabre uma simulação, confere os números recalculados com os preços de hoje e decide transformar aquilo num orçamento congelado para mandar ao cliente. Exige Premium ativo (o botão simplesmente não existe sem isso).
- **Vizinhança imediata:** Na posição (a): acima está o bloco de preços e o botão "Salvar simulação"; abaixo, o fim da página — três botões de "salvar" com significados diferentes na mesma faixa. Na posição (b): acima está o cartão "Kit: {nome}" com o rollup por canal; abaixo, o teaser/seletor de catálogo e o formulário.
- **Dados que chegam (e o que ela devolve):** No caso escalar, congela o resultado do formulário vivo e anexa a procedência `SCENARIO` com o id e o nome da simulação COMO ESTAVAM ao abrir (informativo, nunca fonte de valor). No caso kit, congela o rollup do próprio cartão — os mesmos números que o vendedor está lendo —, jamais os campos intocados do formulário.
- **O que acontece depois:** Abre a folha de registro do Histórico (cliente/pedido), e o orçamento nasce imutável na aba Orçamentos, marcado como originado daquela simulação. A simulação em si não muda: continua aberta, viva e recalculável. Se a rede cair, o registro vai para a fila (outbox) do Histórico — a única escrita da tela que tem esse caminho, ao contrário de tudo em Simulações.
