...
| **E4** | **Immutable price snapshots ("Histórico") + reproducible reprice + export.** **BUILT 2026-07-13..17 (spec 009)**: record online+offline via a durable uid-keyed IndexedDB outbox with exactly-once sync (ADR-0018) · **immutable** snapshots enforced in depth — ORM guard + smuggle-reject + the project's **first PL/pgSQL trigger** — with provenance but no catalog FK (ADR-0019) · the **two-shelf rule** (a snapshot is inert to catalog edit/delete, where a kit degrades — both correct) · "Recalcular hoje" re-resolves the origin at today's catalog and inherits the original's label · US6 manage/lapse over a keyset-paginated **unbounded** ledger · US4 server-rendered PDF/CSV export behind an **active**-entitlement gate (`reportlab==5.0.0`, pinned not assumed — no offline export, by design) · US7 then-vs-now compare (computes no delta). **E4 COMPLETE + SHIPPED to `develop`:** PR-A #18 (`b1fbd80`) + PR-B #19 (`bd9d95e`) + PR-C #20 (`e10b49f`) + close-out #21 (`55f49b6`), all owner-homologated, full CI green (`gate:all` + history e2e 120/120 + contract drift-guard). ADR-0018/0019/0020 Accepted; **A29 closed**. Evidence: `specs/009-e4-history-snapshots-export/dod-evidence.md`. | export = premium (record/reprice/manage all premium; the free calculator stays untouched — SC-512) |
| **E5** | Marketplace simulator — **live multi-channel compute pulled forward into E1 (spec 005, 2026-07-08)**; E5 delivered what was left: **saved scenarios** (per-account live fee auth — Shopee OAuth, AliExpress — deferred OUT to its own increment, spec 010 Q1). **BUILT 2026-07-19..20 (spec 010)**: mutable scenario rows storing INTENT never price (hybrid JSONB config, decimal-string leaves, structural validation on INSERT+UPDATE; ADR-0021 Accepted) · save/consult/offline-read + honest teaser · **the live contract**: read-time resolver D3 live-reflect / D6 last-known degradation for PRODUCT+KIT bases (no FK, owner-scoped) + non-overridden fee slots re-resolving from today's catalog with the "ajustado por você" seal on overrides · duplicate-to-tweak (deep-copy independence) · PUT/PATCH/DELETE/search + lapse read-only · Q12 kit rollup (uniform channelSet → `computeBom`) · **US7 E4 bridge**: record a frozen snapshot from a scenario ("SCENARIO" provenance, payload-side only, immutability untouched). KIT-basis scenario CREATION deferred (owner 2026-07-20, unspecified kit-level channel UX). **E5 COMPLETE + SHIPPED to `develop`:** PR-A #24 (`8386972`) + PR-B #25 (`c9c053b`) + PR-C #26 (`fccc87e`), all owner-homologated (T018 88% · T031 92% · T037 95%), full CI green (gate:all + e2e 70/70 chromium + drift-guard). Evidence: `specs/010-e5-saved-scenarios/dod-evidence.md`. | saved scenarios = premium (reads survive lapse; writes 403) |
| **E6** | **Billing — the purchase turnstile (MP recurring end-to-end + Play flag-ready).** **BUILT 2026-07-22..08-03 (spec 012)**: price → checkout → **server-verified grant** (webhook signature verified BEFORE any DB touch, verify-then-lookup never trust-the-body, exactly-once inbox via `billing_events.event_key` UNIQUE; ADR-0023 **Accepted** at the PR-A gate) · the REVERSE half: cancel-at-period-end · grace & dunning (grace = append-only grant anchored on `current_period_end`, window = `max(MP_RETRY_WINDOW_DAYS=10, GRACE_FLOOR_DAYS=7)` — the RULE is pinned, not the number) · the Conta billing home over a resolved `PlanState` discriminated union (SC-708 structural: the panel CANNOT infer billing state) · US7 four teasers lit with the real price · US8 refund/chargeback revocation (`grant_writer._revoke_for_refund`) · Play Billing readiness behind a server-side flag that is OFF and ASSERTED (404 in every env). The 013 audit hardened the webhook (`data.id` from the QUERY PARAM — MP's real contract) and bounded every grant expiry. **E6 CODE-COMPLETE + SHIPPED to `develop`:** PR-A #28 (`0a3296b`) + PR-B #35 (`26397a5`) + PR-C #36 (`2d3e538`), all owner-merged, full CI green. Built against the LOCAL MP STUB by owner re-sequencing (2026-07-21): T002 (real MP sandbox) + T016b/T018b real-sandbox validation + T036 (Play `purchaseToken` provider) remain **owner-side**. Evidence: `specs/012-e6-billing/dod-evidence.md`. | **actual purchase flow** — the grant is the terminus: one server-side writer fed only by verified events; operator CLI never widened to `source=payment`; no money column, no card data, ever |
