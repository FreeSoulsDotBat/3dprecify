...
| TD-014 | Self-host brand **webfonts** (Inter/Lilita One/Paytone One `.woff2`) into `apps/web` for offline PWA | 25% | Before visual homologation cares about display type, or first public deploy | Token families declared with `system-ui` fallbacks; `tokens/fonts.css` + `assets/fonts/` not yet vendored (titles render system font) |
