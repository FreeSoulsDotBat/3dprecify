...
| TD-020 | **`parseDecimal` silent misparse** — `"1.2"`→12 (10× price error for mixed separators), `"1 234,56"`→1, `"R$ 100,00"`→NaN→0, all silent (only weight has blocking validation). Proper fix needs per-field validation UX (reject + inline error), not just a stricter regex (strict-NaN alone would silently coerce to 0) | 45% | E1 calculator spec (full model rework touches every numeric field) — or sooner if a user reports a wrong price | 001 behavior kept; `computeCalculator` coerces NaN→0 except weight; risk documented by post-close audit 2026-07-03 |
