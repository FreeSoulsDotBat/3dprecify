    ...
    it("toda entrada com bandas passa em `checkBandCoverage` (sem sobreposição, sem inversão)", () => {
        ...
        for (const m of secoes) {
            for (const e of entradasDe(m)) {
                const bandas = e.priceBands as
                    { minPrice: number; maxPrice: number | null }[] | null;
                if (!bandas || bandas.length === 0) continue;
                const v = checkBandCoverage(bandas);
                if (!v.ok)
                    problemas.push(
                        `${String(m.marketplace)} ${JSON.stringify(e.determinants)}: ${v.reason}`,
                    );
            }
        }
        ...
        for (const m of secoes) {
            ...
            for (const e of entradasDe(m)) {
                ...
                if (cat !== undefined && !ids.has(cat))
                    orfas.push(`${String(m.marketplace)}: ${cat}`);
            }
        }
            ...
            for (const e of entradasDe(m)) {
                ...
                for (const v of valores) {
                    if (!(v > 0 && v <= ALIQUOTA_MAXIMA)) {
                        foraDaFaixa.push(
                            `${String(m.marketplace)} ${JSON.stringify(e.determinants)}: ${v}%`,
                        );
                    }
                }
            }
