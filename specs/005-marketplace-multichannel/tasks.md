...
- [x] T028 [US3] The fetch→persist→seed flow already shipped in `use-fee-catalog` (T009c, MVP): first-load fetch → persist to IndexedDB → store thereafter, seed fallback, staleness from `lastReviewed` vs the device clock. **This slice wired the remaining UI**: `calcular-page.tsx` now consumes `isError`/`isFetching`/`refetch` and renders a **non-blocking retry** (`Alert` tone "info", role="status" + a "Tentar novamente" button) in the Marketplace section — the seed/store keep every price live, never a blocking error.
