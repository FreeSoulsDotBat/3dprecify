...
- [x] T024 [P] [US2] `apps/web/tests/e2e/focus-none.spec.ts` (NOVO — a guarda do INVERSO): para cada tipo de controle (botão, link do menu, aba segmentada, switch, campo, dispensa de alerta), focar por Tab e asserir `outline-style: none` e `box-shadow: none` computados; campo mantém `border-color` de acento. Provar não-vácuo restaurando um anel temporariamente → vermelho → reverter. Substitui o medidor de foco do 018 (`_diag-foco`, que fica obsoleto). **Escrita 27/08**; 1ª rodada achou 2 bugs do spec (switch disabled; `has` não-relativo) e a transição de `border-color` (`expect.poll`); não-vácuo por restauração de anel vai na rodada final.
